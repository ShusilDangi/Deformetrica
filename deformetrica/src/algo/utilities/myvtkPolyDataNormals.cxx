/* Deformetrica: This file derives from vtkPolyDataNormals.cxx in the VTK library */
/* It is disributed under the terms of the Inria Non-Commercial License Agreement */

/*=========================================================================

  Program:   Visualization Toolkit
  Module:    myvtkPolyDataNormals.cxx
...
//PP
void computeWeightedTriangleNormal(
  vtkPoints* p,
  vtkIdType* ids,
  double n[3])
{
...
vtkStandardNewMacro(myvtkPolyDataNormals);

// Construct with feature angle=30, splitting and consistency turned on,
// flipNormals turned off, and non-manifold traversal turned on.
myvtkPolyDataNormals::myvtkPolyDataNormals()
{
...
int myvtkPolyDataNormals::RequestData(
      ...
      do {
        ...
        for (cIdx = 0; cIdx < nleftmostCells; cIdx++)
          ...
          if (nCellPts != 3)
            ...
            computeWeightedTriangleNormal(inPts, cellPts, n);
  ...
  for (cellId=0, newPolys->InitTraversal(); newPolys->GetNextCell(npts,pts);
    ...
    if ((cellId % 1000) == 0)
      ...
      if (npts != 3)
        ...
        computeWeightedTriangleNormal(inPts, pts, n);
...
//
void myvtkPolyDataNormals::TraverseAndOrder (void)
{
  vtkIdType p1, p2, i, k;
  int j, l;
...
//
void myvtkPolyDataNormals::MarkAndSplit (vtkIdType ptId)
{
  int i,j;
...
void myvtkPolyDataNormals::PrintSelf(ostream& os, vtkIndent indent)
{
